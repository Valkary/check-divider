...
async function drop_tables_in_dev_mode() {
    try {
        if (process.env.NODE_ENV === 'development') {
            ...
            const tables = await db.execute(sql`SELECT table_schema || '.' || table_name as table_name FROM information_schema.tables WHERE table_type = 'BASE TABLE' AND table_schema NOT IN ('pg_catalog', 'information_schema');`) as { table_name: string }[];
            ...
            for (const { table_name } of tables) {
                ...
                console.info(`==> Successfully dropped table ${table_name}.`);
            }
